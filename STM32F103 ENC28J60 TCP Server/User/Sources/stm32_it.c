...
void USART1_IRQHandler(void)
{
	...
	if (ip_dst_str.buf_len > sizeof(ip_dst_str.ip_buf))
		ip_dst_str.buf_len = 0;
	else if(ans == 'a')
	{
		ip_dst_str.ip_buf[ip_dst_str.buf_len] = 0;
		uart_transmit((uint8_t *)"\r\n", 2);
		uart_transmit((uint8_t *)ip_dst_str.ip_buf, ip_dst_str.buf_len);
		uart_transmit((uint8_t *)"\r\n", 2);
		
		ip_str2num((char *)ip_dst_str.ip_buf, ip_dst);
		sprintf(s1,"%03d.%03d.%03d.%03d\r\n", ip_dst[0], ip_dst[1], ip_dst[2], ip_dst[3]);
		uart_transmit((uint8_t *)s1, strlen(s1));
		
		ip_dst_str.ip_buf[0] = 0;
		ip_dst_str.buf_len = 0;
	...
	else if(ans == 'u')
	{
		...
		dst_prt = port_str2num((char *)ip_dst_str.ip_buf);
		
		ip_dst_str.ip_rec = UART_UDP_RQST;
		ip_dst_str.ip_buf[0] = 0;
		ip_dst_str.buf_len = 0;
	...
	{
		ip_dst_str.ip_buf[ip_dst_str.buf_len] = ans;
		ip_dst_str.buf_len++;
	}
			...
			if(frame.hdr.type == ARP_TYPE)
				{
					...
					if(res == 1)
						arp_reply(&frame, frame.data_len);
					else if (res == 2)
					{
						fill_arp_table(arp_table, &cur_index, ARP_TABLE_LEN, clk_s, &frame);
						show_arp_table(arp_table, cur_index);
						if(ip_dst_str.ip_rec == UART_UDP_RQST)
						{
							i = find_arp_table_data(arp_table, cur_index, ip_dst);
							if(i != -1)
							{
								sprintf(s1, "Hello from UDP-Client to UDP-Server!!!\r\n");
								udp_request(arp_table[i].mac_addr, arp_table[i].ip_addr, dst_prt, (uint8_t *)s1, strlen(s1));
							}
							ip_dst_str.ip_rec = 0;
