...
void net_init(void)
{
	...
	tcp_pckt_prop.tcp_stat = TCP_DISCONNECTED;
...
uint8_t tcp_read(enc28_frame *frame_rec, uint16_t tcp_pckt_len)
{
	...
	if(tcp_msg.hdr->flgs == (TCP_PSH | TCP_ACK))
	{
		if(tcp_msg.data_len)
		{
			...
			if(strncmp((char *)tcp_msg.data, "GET / ", 6) == 0)
			{
				tcp_pckt_prop.data_size = strlen(http_header) + sizeof(index_htm);
				tcp_pckt_prop.data_cnt = tcp_pckt_prop.data_size/TCP_SEG_LEN + 1;
				tcp_pckt_prop.last_data_size = tcp_pckt_prop.data_size % TCP_SEG_LEN;
				
				if(tcp_pckt_prop.data_cnt == 1)
				{
					tcp_pckt_prop.data_stat = TCP_DATA_ONE;
					
				}
				else if(tcp_pckt_prop.data_cnt > 1)
				{
					tcp_pckt_prop.data_stat = TCP_DATA_FIRST;
				}
				
				if(tcp_pckt_prop.data_stat == TCP_DATA_ONE)
					tcp_http_data_send_one(frame_rec, &tcp_pckt_prop, tcp_pckt_len);
	...
	else if (tcp_msg.hdr->flgs == TCP_ACK)
	{
		...
		uart_transmit((uint8_t *)s1, strlen(s1));
		if(tcp_pckt_prop.data_stat == TCP_DATA_END)
			tcp_http_data_end(frame_rec, &tcp_pckt_prop);
...
{
	tcp_repl->hdr->dst_prt = be16toword(tcp_prop->dst_port);
	tcp_repl->hdr->src_prt = be16toword(TCP_PORT_SRC);
	tcp_repl->hdr->sn = be32todword(tcp_prop->sn);
	tcp_repl->hdr->ack_sn = be32todword(tcp_prop->ack_sn);
	tcp_repl->hdr->flgs = flgs;
	tcp_repl->hdr->hdr_len = (uint8_t)(tcp_header_len << 2);
	tcp_repl->hdr->win_size = be16toword(TCP_WIN_LEN);
	tcp_repl->hdr->urg_ptr = 0;
	tcp_repl->hdr->ch_sum = 0;
	tcp_repl->hdr->ch_sum = ip_ch_sum_calc((uint8_t*)tcp_repl->hdr - 8, tcp_data_len + tcp_header_len + 8, TCP_CH_SUM);
...
void tcp_reply(enc28_frame *frame_rec, uint8_t op_type, uint16_t tcp_pckt_len)
{
	...
	if(op_type == TCP_OP_SYNACK)
	{
		tcp_pckt_prop.ack_sn = be32todword(tcp_repl.hdr->sn)+1;
		tcp_pckt_prop.sn = 1;
		tcp_pckt_prop.dst_port =  be16toword(tcp_repl.hdr->src_prt);
		...
		tcp_header_prepare(&tcp_repl, &tcp_pckt_prop, (TCP_SYN | TCP_ACK), sizeof(tcp_header) + tcp_repl.data_len, 0);
	
		ip_reply(&frame_repl, tcp_repl.data_len + sizeof(tcp_header));
		sprintf(s1,"SYN ACK transmitted\r\n");
		uart_transmit((uint8_t *)s1, strlen(s1));
		
		tcp_pckt_prop.tcp_stat = TCP_CONNECTED;
		
	}
	else if(op_type == TCP_OP_ACK_OF_FIN)
	{
		tcp_pckt_prop.ack_sn = be32todword(tcp_repl.hdr->sn)+1;
		tcp_pckt_prop.sn = be32todword(tcp_repl.hdr->ack_sn);
		tcp_pckt_prop.dst_port =  be16toword(tcp_repl.hdr->src_prt);
		
		tcp_header_prepare(&tcp_repl, &tcp_pckt_prop, TCP_ACK, sizeof(tcp_header), tcp_repl.data_len);
		
		ip_reply(&frame_repl, tcp_repl.data_len + sizeof(tcp_header));
		sprintf(s1,"ACK for FIN | ACK transmitted\r\n");
		uart_transmit((uint8_t *)s1, strlen(s1));
		
		if(tcp_pckt_prop.tcp_stat != TCP_DISCONNECTED)
		{
			memcpy((void *)&frame_repl.hdr, (void *)&(frame_rec->hdr), sizeof(enc28_frame_header)); 
			memcpy(frame_repl.data, (void*)ip_msg.hdr, sizeof(ip_header));
			tcp_header_prepare(&tcp_repl, &tcp_pckt_prop, (TCP_FIN | TCP_ACK), sizeof(tcp_header), tcp_repl.data_len);
			ip_reply(&frame_repl, tcp_repl.data_len + sizeof(tcp_header));
	...
	{
		tcp_data_rec++;
		tcp_pckt_prop.ack_sn = be32todword(tcp_repl.hdr->sn) + tcp_pckt_len - sizeof(tcp_header);
		tcp_pckt_prop.sn = be32todword(tcp_repl.hdr->ack_sn);
		tcp_pckt_prop.dst_port =  be16toword(tcp_repl.hdr->src_prt);
		tcp_header_prepare(&tcp_repl, &tcp_pckt_prop, TCP_ACK, sizeof(tcp_header), tcp_repl.data_len);
		...
		if(tcp_data_rec == 2)
		{
			...
			tcp_header_prepare(&tcp_repl, &tcp_pckt_prop, (TCP_PSH | TCP_ACK), sizeof(tcp_header), tcp_repl.data_len);
			ip_reply(&frame_repl, tcp_repl.data_len + sizeof(tcp_header));
...
void tcp_http_data_send_one(enc28_frame *frame_rec, tcp_prop *tcp_prop, uint16_t tcp_pckt_len)
{
	...
	tcp_prop->ack_sn = be32todword(tcp_repl.hdr->sn)+ tcp_pckt_len - sizeof(tcp_header);
	tcp_prop->sn = be32todword(tcp_repl.hdr->ack_sn);
	tcp_prop->dst_port =  be16toword(tcp_repl.hdr->src_prt);	
	tcp_header_prepare(&tcp_repl, tcp_prop, TCP_ACK, sizeof(tcp_header), tcp_repl.data_len);
	
	ip_reply(&frame_repl, sizeof(tcp_header));
	free(frame_repl.data);
	sprintf(s1,"HTTP Request ACK transmitted\r\n");
	uart_transmit((uint8_t *)s1, strlen(s1));
		
	// Data transmission
	free(frame_rec->data + sizeof(ip_header) + sizeof(tcp_header));
	
	tcp_repl.data_len = strlen(http_header) + sizeof(index_htm);
	memcpy((void *)&frame_repl.hdr, (void *)&(frame_rec->hdr), sizeof(enc28_frame_header));
	tcp_repl.hdr = (void *)ip_msg.data;	
	frame_repl.data = (uint8_t*) malloc((sizeof(ip_header) + sizeof(tcp_header) + tcp_repl.data_len) * sizeof(uint8_t));
	memcpy(frame_repl.data, (void*)ip_msg.hdr, sizeof(ip_header));
	memcpy(frame_repl.data + sizeof(ip_header), (void*)tcp_repl.hdr, sizeof(tcp_header));
	memcpy(frame_repl.data + sizeof(ip_header) + sizeof(tcp_header), (void*)http_header, strlen(http_header));
	memcpy(frame_repl.data + sizeof(ip_header) + sizeof(tcp_header) + strlen(http_header), (void*)index_htm, sizeof(index_htm));
	tcp_repl.hdr = (void*)(frame_repl.data + sizeof(ip_header));
	tcp_repl.data = frame_repl.data + sizeof(ip_header) +  sizeof(tcp_header);
		
	tcp_header_prepare(&tcp_repl, tcp_prop, (TCP_PSH | TCP_ACK), sizeof(tcp_header), tcp_repl.data_len);
	tcp_prop->data_stat = TCP_DATA_END;
	
	ip_reply(&frame_repl, tcp_repl.data_len + sizeof(tcp_header));
	free(frame_repl.data);
	sprintf(s1,"HTTP DATA transmitted\r\n");
	uart_transmit((uint8_t *)s1, strlen(s1));
...
void tcp_http_data_end(enc28_frame *frame_rec, tcp_prop *tcp_prop)
{
	...
	tcp_prop->ack_sn = be32todword(tcp_repl.hdr->sn);
	tcp_prop->sn = be32todword(tcp_repl.hdr->ack_sn);
	tcp_prop->dst_port =  be16toword(tcp_repl.hdr->src_prt);	
	
	tcp_header_prepare(&tcp_repl, tcp_prop, (TCP_FIN | TCP_ACK), sizeof(tcp_header), tcp_repl.data_len);
	
	ip_reply(&frame_repl, tcp_repl.data_len + sizeof(tcp_header));
	free(frame_repl.data);
	
	sprintf(s1,"HTTP FIN ACK transmitted\r\n");
	uart_transmit((uint8_t *)s1, strlen(s1));
	
	tcp_prop->data_stat = TCP_DATA_COMPLETED;
	tcp_prop->tcp_stat = TCP_DISCONNECTED;
