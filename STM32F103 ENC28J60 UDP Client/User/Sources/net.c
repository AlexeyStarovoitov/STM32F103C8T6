...
void udp_reply(enc28_frame *frame_rec)
{
	...
	udp_repl.hdr->ch_sum = ip_ch_sum_calc(frame_repl.data + sizeof(ip_header) - 8, udp_repl.data_len + sizeof(udp_header) + 8, 1);
...
void udp_request(uint8_t *mac_dest, uint8_t *ip_dest, uint16_t port_dest, uint8_t *data, uint16_t dat_len)
{
	...
	udp_req.hdr->ch_sum = ip_ch_sum_calc((void*)(udp_req.data - sizeof(udp_header)-8), udp_req.data_len + sizeof(udp_header) + 8, 1);
