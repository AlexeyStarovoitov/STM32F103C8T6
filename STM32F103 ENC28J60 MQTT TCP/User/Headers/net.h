...
#define TCP_SEG_LEN ((uint16_t)33285)

// Check Sum
#define UDP_CH_SUM ((uint8_t)1)
#define TCP_CH_SUM ((uint8_t)2)

#define MAC_ADDR_SRC {0x00,0x15,0x42,0xBF,0xF0,0x51};
#define MAC_ADDR_NULL {0x00,0x00,0x00,0x00,0x00,0x00};
#define MAC_ADDR_BC {0xFF,0xFF,0xFF,0xFF,0xFF,0xFF};
#define IP_ADDR_SRC {192,168,137,2};
...
typedef struct
{
	...
	uint8_t ip_addr_src[4];
	uint8_t ip_addr_dst[4];
	uint16_t udp_src_prt, udp_dst_prt;
	uint16_t tcp_src_prt, tcp_dst_prt;
	uint32_t tcp_seq_num, tcp_ack_num;
	enc28_frame frame_rec, frame_repl;
