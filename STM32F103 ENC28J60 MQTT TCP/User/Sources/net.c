...
void net_init(net_socket * socket)
{
	uint16_t i ;
	socket->frame_req.data = socket->frame_repl.data = NULL;
	socket->frame_req.data_len = socket->frame_repl.data_len = 0;
	
	memecpy(socket->mac_addr_src, MAC_ADDR_SRC, sizeof(MAC_ADDR_SRC));
	memset(socket->mac_addr_dst, 0, sizeof(*socket->mac_addr_dst));
	memecpy(socket->ip_addr_src, IP_ADDR_SRC, sizeof(IP_ADDR_SRC));
	memset(socket->ip_addr_dst, 0, sizeof(*socket->ip_addr_dst));
	
	socket->udp_src_prt = socket->udp_dst_prt = 0;
	socket->tcp_src_prt = socket->tcp_dst_prt = 0;
	socket->tcp_seq_num = socket->tcp_ack_num = 0;
	
	socket->ip_dst_str.buf_len = 0;
	socket->ip_dst_str.ip_rec = 0;
	
	socket->cur_index = 0;
	socket->clk_s = 0;
	memset(socket->arp_table, 0, sizeof(*socket->arp_table));
...
void eth_reply(net_socket * socket, uint16_t dat_len)
//void eth_reply(enc28_frame *frame_repl, uint16_t dat_len)
{
	memcpy(socket->frame_repl.hdr.dest_addr,   socket->mac_addr_dst, sizeof(*socket->mac_addr_dst));
	memcpy(socket->frame_repl.hdr.src_addr, socket->mac_addr_src, sizeof(*socket->mac_addr_src));
	
	socket->frame_repl.data_len = dat_len; 
	enc28_packet_transmit(socket->frame_repl);
}

void eth_request(net_socket * socket, uint16_t prot_type, uint16_t dat_len)
//void eth_request(enc28_frame *frame_req, uint16_t prot_type, uint8_t *mac_dest, uint16_t dat_len)
{
	
	memcpy(socket->frame_req.hdr.dest_addr,   socket->mac_addr_dst, sizeof(*socket->mac_addr_dst));
	memcpy(socket->frame_req.hdr.src_addr, socket->mac_addr_src, sizeof(*socket->mac_addr_src));

	socket->frame_req.hdr.type = prot_type;
	socket->frame_req.data_len = dat_len; 
	enc28_packet_transmit(socket->frame_req);
}


//void arp_reply(enc28_frame *frame_rec,  uint16_t arp_pckt_len)
void arp_reply(net_socket * socket,  uint16_t arp_pckt_len)
{
	arp_pckt *arp_repl = (void*)socket->frame_rec.data;
	
	
	memcpy(socket->mac_addr_dst, arp_repl->mac_src, sizeof(arp_repl->mac_src));
	memcpy(socket->mac_addr_src, MAC_ADDR_SRC, sizeof(MAC_ADDR_SRC));
	memcpy(socket->ip_addr_dst, arp_repl->ip_src, sizeof(arp_repl->ip_src));
	memcpy(socket->ip_addr_src, IP_ADDR_SRC, sizeof(IP_ADDR_SRC));
	
	
	arp_repl->req_type = ARP_REP;
	memcpy(arp_repl->mac_dest, socket->mac_addr_dst, sizeof(socket->mac_addr_dst));
	memcpy(arp_repl->mac_src, socket->mac_addr_src, sizeof(socket->mac_addr_src));
	memcpy(arp_repl->ip_dest, socket->ip_addr_dst, sizeof(socket->ip_addr_dst));
	memcpy(arp_repl->ip_src, socket->ip_addr_src, sizeof(socket->ip_addr_src));
	
	eth_reply(socket, sizeof(arp_pckt));
	
}

uint8_t arp_request (net_socket * socket, uint8_t *ip_dst)
{
	//enc28_frame frame_req;
	socket->frame_req.data = (uint8_t*) calloc(sizeof(arp_pckt), sizeof(*socket->frame_req.data));
	arp_pckt *arp_req = (void *)socket->frame_req.data;
	
	memcpy(socket->mac_addr_dst, MAC_ADDR_BC, sizeof(MAC_ADDR_BC));
	memcpy(socket->mac_addr_src, MAC_ADDR_SRC, sizeof(MAC_ADDR_SRC));
	memcpy(socket->ip_addr_dst, ip_dst, sizeof(ip_dst));
	memcpy(socket->ip_addr_src, IP_ADDR_SRC, sizeof(IP_ADDR_SRC));
	
	arp_req->net_type = ARP_ETH;
	arp_req->prot_type = ARP_IP;
	arp_req->mac_len = sizeof(socket->mac_addr_src);
	arp_req->ip_len = sizeof(socket->ip_addr_src);
	arp_req->req_type = ARP_REQ;
	memcpy(arp_req->mac_dest, MAC_ADDR_NULL, sizeof(MAC_ADDR_NULL));
	memcpy(arp_req->mac_src, socket->mac_addr_src, sizeof(socket->mac_addr_src));
	memcpy(arp_req->ip_dest, socket->ip_addr_dst, sizeof(socket->ip_addr_dst));
	memcpy(arp_req->ip_src, socket->ip_addr_src, sizeof(socket->ip_addr_src));

	
	eth_request(socket, ARP_TYPE, sizeof(arp_pckt));
	free(socket->frame_req.data);
...
uint8_t arp_read(net_socket * socket, uint16_t len)
{
	uint8_t res = 0;
	char s1[STR_LEN];
	
	arp_pckt *arp_mess = (void *)(socket->frame_rec.data);
	if(((arp_mess->net_type) == ARP_ETH) && (arp_mess->prot_type == ARP_IP) && (len >= sizeof(arp_pckt))&& (!memcmp(arp_mess->ip_dest, IP_ADDR_SRC, 4)))
	{
...
uint8_t ip_reply(net_socket * socket, uint16_t ip_dat_len)
{
	ip_pckt ip_msg; 
	uint8_t res = 0;
	
	ip_msg.hdr = (void *)socket->frame_repl.data;
	ip_msg.data_len = ip_dat_len;
	
	memcpy(socket->mac_addr_dst, socket->frame_repl.hdr.src_addr, sizeof(socket->frame_repl.hdr.src_addr));
	memcpy(socket->mac_addr_src, MAC_ADDR_SRC, sizeof(MAC_ADDR_SRC));
	memcpy(socket->ip_addr_dst, ip_msg.hdr->ip_src, sizeof(socket->ip_addr_dst));
	memcpy(socket->ip_addr_src, IP_ADDR_SRC, sizeof(IP_ADDR_SRC));
	
	
	ip_msg.hdr->flgs_fr_off = 0;
	ip_msg.hdr->ttl = 128;
	memcpy(ip_msg.hdr->ip_dest, socket->ip_addr_dst,sizeof(socket->ip_addr_dst));
	memcpy(ip_msg.hdr->ip_src, socket->ip_addr_src,sizeof(socket->ip_addr_src));
...
void ip_request(net_socket * socket, uint8_t prot, uint8_t *mac_dest, uint8_t *ip_dest, uint16_t dat_len)
{
	ip_pckt ip_req;
	
	ip_req.data_len = dat_len;
	ip_req.data = socket->frame_req.data + sizeof(ip_header);
	ip_req.hdr = (void*)(socket->frame_req.data);
	
	memcpy(socket->mac_addr_dst, mac_dest, sizeof(socket->mac_addr_dst));
	memcpy(socket->mac_addr_src, MAC_ADDR_SRC, sizeof(MAC_ADDR_SRC));
	memcpy(socket->ip_addr_dst, ip_dest, sizeof(socket->ip_addr_dst));
	memcpy(socket->ip_addr_src, IP_ADDR_SRC, sizeof(IP_ADDR_SRC));
	...
	memcpy(ip_req.hdr->ip_dest, socket->ip_addr_dst, sizeof(socket->ip_addr_dst));
	memcpy(ip_req.hdr->ip_src, socket->ip_addr_src, sizeof(socket->ip_addr_src));
	
	ip_req.hdr->ch_sum = 0;
	ip_req.hdr->ch_sum = ip_ch_sum_calc((uint8_t *)ip_req.hdr, sizeof(ip_header), 0);
	
	eth_request(socket, IP_TYPE, ip_req.data_len + sizeof(ip_header));
	
}

//uint8_t ip_read(enc28_frame *frame_rec, uint16_t ip_len)
uint8_t ip_read(net_socket * socket, uint16_t ip_len)
{
	uint8_t res = 0;
	char s1[STR_LEN];
	...
	ip_pckt ip_msg;
	ip_msg.hdr = (void *)socket->frame_repl.data;
	ip_msg.data = socket->frame_repl.data + sizeof(ip_header);

	if((ip_len > sizeof(ip_header)) && (ip_msg.hdr->ver_hlen == IP_VERLEN) && (!memcmp(ip_msg.hdr->ip_dest, IP_ADDR_SRC, sizeof(IP_ADDR_SRC))))
	{
		...
		ip_msg.hdr->ch_sum = 0;
		ip_msg.hdr->ch_sum = ip_ch_sum_calc((uint8_t *)socket->frame_rec.data, sizeof(ip_header), 0);
		sprintf(s1, "ip ch_sum calc: %04X\r\n", ip_msg.hdr->ch_sum);
		uart_transmit((uint8_t *)s1, strlen(s1));
		
		if(ip_msg.hdr->prot_id == IP_TCP)
		{
			tcp_read(socket, ip_msg.data_len);
		}
		else if (ip_msg.hdr->prot_id == IP_UDP)
		{
			if(udp_read(socket, ip_msg.data_len))
				udp_reply(socket);
			
		}
		else if (ip_msg.hdr->prot_id == IP_ICMP)
		{
			if(icmp_read(socket, ip_msg.data_len))
				icmp_reply(socket, ip_msg.data_len);
...
uint8_t icmp_read(net_socket * socket, uint16_t icmp_pckt_len)
{
	uint8_t res=0;
	char s1[STR_LEN];
	ip_pckt ip_msg;
...
void icmp_reply(net_socket * socket, uint16_t icmp_pckt_len)
{
	ip_pckt ip_msg;
	icmp_pckt icmp_msg;
	...
	ip_reply(socket, icmp_pckt_len);
	
}


uint8_t udp_read(net_socket * socket, uint16_t udp_pckt_len)
{
	uint8_t res = 0;
	char s1[STR_LEN];
	ip_pckt ip_msg;
...
void udp_reply(net_socket * socket)
{
	...
	memcpy((void *)&frame_repl.hdr, (void *)&(frame_repl->hdr), sizeof(enc28_frame_header)); 
